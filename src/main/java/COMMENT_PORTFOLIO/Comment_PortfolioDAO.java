...
public class Comment_PortfolioDAO {
	private Connection conn;
	private ResultSet rs;
	
	public Comment_PortfolioDAO() { //데이터 접근 객체
		try {
	...
	public ArrayList<Comment_Portfolio> comment_getList(int bbsID){
		String SQL = "SELECT * FROM COMMENT_PORTFOLIO WHERE COMMENT_bbsID = ? AND COMMENT_Available = 1 ORDER BY COMMENT_NUM DESC"; 
		//bbsID가 특정일 때, 그리고 댓글이 삭제가 되지 않았을 때
		ArrayList<Comment_Portfolio> list = new ArrayList<Comment_Portfolio>();
		try {
			PreparedStatement pstmt = conn.prepareStatement(SQL);
			pstmt.setInt(1, bbsID);
			rs = pstmt.executeQuery();
			while (rs.next()) { 
				Comment_Portfolio comment_portfolio = new Comment_Portfolio();
				comment_portfolio.setCOMMENT_NUM(rs.getInt(1)); 
				comment_portfolio.setCOMMENT_bbsID(rs.getInt(2));
				comment_portfolio.setCOMMENT_userID(rs.getString(3));
				comment_portfolio.setCOMMENT_date(rs.getString(4));
				comment_portfolio.setCOMMENT_comment(rs.getString(5));
				comment_portfolio.setCOMMENT_Available(rs.getInt(6));
				list.add(comment_portfolio); //게시글 목록을 담아서 리스트에 인스턴스 담아서 반환
			}
		}
		catch (Exception e){
			e.printStackTrace();
